...
import networkx as nx  # This is used to create and manipulate networks
...
class AdoptionModel(Model): # Everything idented inside the class is part of the class. Remember, Mesa is OOP, so the model itself is an object
    ...
    def __init__(self, num_agents, seed=None): #_init_ means the model is being intialised
        ...
        self.datacollector = DataCollector(
            model_reporters={
                "Num_Adopters": lambda m: self.count_adoption_stage("E. Has an effective WTP")
            },
            agent_reporters={
    ...
    def build_network(self): # This is a helper methods, to construct my structure
        ...
        self.firm_data = pd.read_excel("Test_Data.xlsx", sheet_name="Firms") # This loads my data on firm types
        ...
        for _, row in self.firm_data.iterrows():
            ...
            G.add_node(firm_id, postcode=postcode, sector=sector, network=network, size=size)  # Add nodes with attributes
